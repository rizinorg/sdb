...
void snprint_mem(char *out, size_t out_size, const char *buf, size_t len) {
	size_t i;
	*out = '\0';
	for (i = 0; i < len; i++) {
		size_t out_len;
...
#define mu_assert_eq(actual, expected, message) \
	do { \
		...
		if ((exp__) != (act__)) { \
			char _meqstr[MU_BUF_SIZE]; \
			snprintf(_meqstr, MU_BUF_SIZE, "%s: expected %"LLFMT"d, got %"LLFMT"d.", (message), (long long)(exp__), (ut64)(act__)); \
			mu_assert(_meqstr, false); \
		} \
	} while (0)
...
#define mu_assert_neq(actual, expected, message) \
	do { \
		...
		ut64 act__ = (ut64)(actual); \
		ut64 exp__ = (ut64)(expected); \
		snprintf(_meqstr, MU_BUF_SIZE, "%s: expected not %"LLFMT"d, got %"LLFMT"d.", (message), (long long)(exp__), (act__)); \
